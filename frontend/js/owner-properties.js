...
async function handleAction(action, id, btn) {
  switch (action) {
    ...
    case "share":
      ...
      break;
    case "students":
      window.location.href = `owner-students.html?propertyId=${id}`;
      break;
    case "reviews":
      window.location.href = `owner-reviews.html?propertyId=${id}`;
      break;
    case "delete":
...
function openShare(id) {
  ...
  if (!property) return;
  const url = `${window.location.origin}${window.location.pathname.replace(/owner-properties\.html$/, "")}property-details.html?id=${id}`;
